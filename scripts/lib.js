...
function buildSupplementRuntime(ctx) {
  ...
  return [
    ...
    "function applyAttrs(root){for(const element of queryAll(root,attrSelector())){for(const attr of attrNames){const value=element.getAttribute(attr);if(!value)continue;const translated=translate(value);if(translated!==value)element.setAttribute(attr,translated);}}}",
    ...
    "function processRoot(root){if(!root)return;observeRoot(root);applyText(root);applyAttrs(root);forEachShadowRoot(root,processRoot)}",
    "let queued=false;",
    "function tick(){if(queued||!ready())return;queued=true;requestAnimationFrame(()=>{queued=false;processRoot(document);processRoot(document.querySelector(\"main\")||document.body);});}",
