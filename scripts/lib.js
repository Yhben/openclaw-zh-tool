...
function buildSupplementRuntime(ctx) {
  ...
  return [
    ...
    "function normalizeLookup(text){return(text||\"\").replace(/[“”]/g,'\"').replace(/[‘’]/g,\"'\").replace(/[–—]/g,'-').replace(/\\u00a0/g,' ').replace(/\\s+/g,' ').trim()}",
    "function translate(text){if(!text||!/[\\u0041-\\u005A\\u0061-\\u007A]/.test(text))return text;const trimmed=text.trim();const direct=exact.get(trimmed);if(direct)return text.replace(trimmed,direct);const normalized=normalizeLookup(trimmed);if(normalized&&exact.has(normalized))return text.replace(trimmed,exact.get(normalized));return text;}",
